...
public abstract class Car {
    ...
    protected boolean isMoving = false; // Указатель, движется ли машина
    protected boolean isInService = false; // Указатель, находится ли машина на обслуживании
    protected int gearNum = 0; // Текущая передача (0 - нейтральная)
    protected boolean lightIsOn = false; // Указатель, включены ли фары
    protected boolean wipersIsOn = false; // Указатель, включены ли дворники
    protected String moveMode = "Передвижение по земле"; // Тип движения (полёт, плавание и т.п.)
    ...
    public void move(boolean isMoving) {
        ...
        if (this.isMoving == isMoving) {
            System.out.println("Машина и так уже "+(isMoving ? "движется" : "стоит")+".");
            return;
        }

        if (isMoving && this.gearNum != 1) {
            System.out.println("Перед началом движения необходимо включить первую скорость.");
            return;
        }

        if (!isMoving && this.gearNum != 0) {
            System.out.println("Перед остановкой машины необходимо сбросить скорость на нейтралку.");
            return;
        }
